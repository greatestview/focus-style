...
- *Four layer separation:* Loaded from low to high:
	...
	2. `/src/2-base-elements/`: minimal html reset and element definition.
	3. `/src/3-utilities/`: handpicked helper classes, especially useful for layout and typography. Ask yourself twice before introducing a new helper class and increasing code complexity/component dependence.
	4. `/src/4-components/`: some common page elements, optional.
- *Typography scale:* Free your mind of font size values, they’re not needed anymore. Just define the font size root in `--fs-font-size-root.css` (or use the default one) and choose a typography scale in `--fs-font-size-scale.css`. That’s it! Now all font sizes for headlines and such are calculated automatically. In addition: If you want to align different font families visually for advanted typography, you can do so by defining an x-height-scale for each font family in `--fs-font-size-x-height-scale.css`.
- *Color ramps:* Ramps are based on the [universal color palette system](https://uxdesign.cc/the-universal-color-palette-9826deb94f7). The color lightness for `--_fs-color--*--500` should be equal across all color ramps. The ramps only contain the steps actual in use, so that they can be extended later without refactoring, if necessary.
- *Semantic color token:* This is the public interface for components and utilities to consume, e.g. `--fs-color--text`. They reference private primitives. Please always use semantic tokens, never literal color values!
