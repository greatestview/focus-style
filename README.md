...
- FS uses a flexible and well-grown **color system**: Tints, shades and alpha ramps derive from a few anchor colors. You can also choose between solid ramps (example in `--_fs-color--neutral.css`) or alpha ramps (default).
- FS is **dark mode** ready: Semantic color tokens switch via `light-dark()` by default. You can even re-theme a whole subtree by setting `color-scheme` on it.
...
- *`--default` marks an overridable alias:* A bare token like `--fs-font-size-scale` is a singleton: one global value, not meant to be overridden. A `--default` variant like `--fs-border-radius--default` is a convenience alias pointing at one of several variants and *may* be re-pointed to another one. If a group has no `--default`, there is no canonical choice and you have to pick a variant explicitly.
...
- *Public knobs for customization:* Un-prefixed custom properties consumed by elements and utilities (e.g. `--fs-gap--space`, `--fs-blockquote-font-size`) are deliberate configuration knobs, meant to be set from outside. The `--fs-blockquote-*` knobs are not declared by the element itself, only read via `var()` fallbacks. So you can set them on any ancestor, and an empty value (`--fs-blockquote-font-size: ;`) switches back to inheritance.
